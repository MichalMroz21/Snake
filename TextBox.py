import pygame


class TextBox():
    def __init__(self, x, y, width, height, text_rect, color, colorPicked, font, initialText, id):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
        self.text_rect = text_rect
        self.color = color
        self.colorPicked = colorPicked
        self.text = initialText
        self.font = font

        self.id = id

        self.TEXT_SURFACE = self.font.render(self.name, True, (255, 255, 255))
        self.TEXT_WIDTH, self.TEXT_HEIGHT = self.font.size(self.name)

        self.TEXT_RECT = pygame.Rect(self.x, self.y, self.width, self.height)

    def update(self, text):

        self.text = text

        self.TEXT_SURFACE = self.font.render(self.text, True, (255, 255, 255))
        self.TEXT_WIDTH, self.TEXT_HEIGHT = self.font.size(self.text)

    def drawNotPickedRectangle(self, screen):
        pygame.draw.rect(screen, self.color, self.TEXT_RECT, 2)

    def drawPickedRectangle(self, screen):
        pygame.draw.rect(screen, self.colorPicked, self.TEXT_RECT, 2)

    def blitText(self, screen):
        screen.blit(self.TEXT_SURFACE, (self.TEXT_RECT.x + self.width / 2 - self.TEXT_WIDTH / 2, self.TEXT_RECT.y + self.height / 2 - self.font.get_height() / 2))

    def checkIfClicked(self, mousePosX, mousePosY):
        if mousePosX >= self.RIGHT_RECT.x and mousePosX <= self.RIGHT_RECT.x + self.RIGHT_RECT.width and mousePosY >= self.RIGHT_RECT.y and mousePosY <= self.RIGHT_RECT.y + self.RIGHT_RECT.height:
            return id

        else: return -1